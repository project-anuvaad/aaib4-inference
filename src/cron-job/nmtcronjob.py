...
class NMTcronjob(Thread):
    ...
    def run(self):
        ...
        while not self.stopped.wait(nmt_cron_interval_ms):
            try:
                    ...
                    for i in range(0, sub_df.shape[0], translation_batch_limit):
                        ...
                        if 'tgt_list' in output:
                            for i,tgt_sent in enumerate(output):
                                ...
                                log_info("Final output from Async call for ULCA batch translation pushed on redis for a request : {}".format(final_output),MODULE_CONTEXT)  
                                redis_client.upsert(db_key_list[i], final_output)
                ...
                run += 1
                log_exception("Async ULCA Batch Translation Cron-job" + " -- Run: " + str(run) + " | Exception in Cornjob: " + str(e), e,)
    ...
    def check_schema_ULCA(self, json_ob):
        ...
        if len(json_ob) > 0 and all(v in  json_ob for v in ['input','config']) and\
            ...
            if all(j in json_ob.get('config')['language'] for j in ['sourceLanguage', 'targetLanguage']) and \
                ...
                return [json_ob, True, json_ob.get('input')[0]['source'], json_ob.get('config')['modelId'],json_language['sourceLanguage'], json_language['targetLanguage']]
    ...
    def create_dataframe(self, redis_data):
        ...
        json_df = pd.DataFrame(columns = ['input','schema','sentence','modelid','src_language','tgt_language','db_key'])
        for key,value in redis_data:
            chk = self.check_schema_ULCA(value)
            chk.append(key)
            json_df.loc[len(json_df)] = chk
        json_df = json_df.astype({'sentence':str, 'db_key':str, 'src_language': str, 'tgt_language': str}, errors='ignore')
        return json_df
