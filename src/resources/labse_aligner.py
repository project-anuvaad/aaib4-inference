...
from services import LabseAlignerService, LabseAlignerWithModelAttentionService
...
class LabseAlignerWithModelAttentionResource(Resource):
    def post(self):
        inputs = request.get_json(force=True)
        response_list = list()
        if len(inputs)>0:
            ...
            try:
                for i in inputs:
                    log_info(i, MODULE_CONTEXT)
                    if all(v in i for v in ["src","src_phrases","tgt"]):
                        log_info("Making labse-aligner service call",MODULE_CONTEXT)
                        res = LabseAlignerWithModelAttentionService.phrase_aligner(i)
                        response_list.append(res)
                        out = CustomResponse(Status.SUCCESS.value, response_list)                    
                    else:
