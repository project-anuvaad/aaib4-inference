...
def dhruva_api_call(src_list, source_language_code, target_language_code):
	...
	try:	
		...
		for i in range(0,max_retries):
			...
			log_info("Dhruva returned content {0}-{1} in time {2} |".format(response.status_code, response.text, call_time), MODULE_CONTEXT)
			if response.status_code >=200 and response.status_code <=204:
				break
			else:
				log_info(f"Dhruva API Request has beed called, Not success {response.status_code}-{response.text} in trial {i} out of {max_retries} for input {data_json}|", MODULE_CONTEXT)
				time.sleep(30)
